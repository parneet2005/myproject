...
create_training_data_for_regression_model = 0;

if (create_training_data_for_regression_model==1)

    loop = 10; %number of iterations for different vehicle positions % this cannot be zero!
    regression = 2; %number of iterations for different vehicle positions
    make_regression = 1; %test for regression model data
    same_data = 1;

elseif (create_training_data_for_regression_model==0)
...
if (same_data ==1 && scenario==0)

    load('Initial_vehicle_positions_data_sameside.mat')

elseif (same_data ==1 && scenario ==1)

    load('Initial_vehicle_positions_data_opposingside.mat')

end

if (raytracing==1 && scenario==0)

    if (step ==0.1)

        load('regression_data_raytracing_sameside_sub_ns.mat')

    elseif (step==1)

        load('regression_data_raytracing_sameside_ns.mat')

    end


elseif (raytracing==1 && scenario==1)

    if (step==0.1)

        load('regression_data_raytracing_opposingside_sub_ns.mat')

    elseif (step==1)

        load('regression_data_raytracing_opposingside_ns.mat')

    end

elseif (raytracing==0 && scenario==0)

    if (step==0.1)

        load('regression_data_sameside_sub_ns.mat')

    elseif (step==1)

        load('regression_data_sameside_ns.mat')

    end

elseif (raytracing==0 && scenario==1)

    if (step==0.1)

        load('regression_data_opposingside_sub_ns.mat')

    elseif (step==1)

        load('regression_data_opposingside_ns.mat')
...
%Tri_TDOA method
 [Z,d] = Tri_tdoa(orderx,ordery,receiver_times,c,ls,weights);
...
if (plot_graph ==1)
    ...
    scenario1 = drivingScenario('SampleTime',0.1','StopTime',60);

    p1=scatter([final_A(1), final_B(1)],[final_A(2), final_B(2)], 'filled', 'r');
    
    car = vehicle(scenario1,'ClassID',1,'Position',[final_A(1)  final_B(1) 0], ...
    'Length',toyota_length,'Width',toyota_width,'Height',1.6);
    %receiver points
    plot(scenario1, car);
